...
void EnvironmentAudio::SetCityAmbienceMuted(bool muted) {
    cityAmbienceMuted = muted;
}
...
void EnvironmentAudio::Update(const EnvironmentFrame& frame, EnvironmentSystem& environment) {
    ...
    const float cityVolume = cityAmbienceMuted ? 0.0f : 1.0f;
    ma_sound_set_volume(&cityDay, cityVolume * (1.0f - nightMix) * 0.22f);
    ma_sound_set_volume(&cityNight, cityVolume * nightMix * 0.25f);
