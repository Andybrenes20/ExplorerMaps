...
namespace {
    ...
    const glm::vec3 kTrafficCollisionSize(2.35f, 1.55f, 4.75f);
    ...
    // Rutas de trafico con coordenadas fijas del mapa.
...
bool TrafficSystem::IsLaneClear(const TrafficCar& car, PhysicsWorld& city) const {
    ...
    for (float distance : sampleDistances) {
        ...
        if (!city.RaycastStatic(origin, glm::vec3(0.0f, -1.0f, 0.0f), groundDistance) || groundDistance > 12.0f) {
            return false;
        }
    ...
    return !city.RaycastStatic(car.position + glm::vec3(0.0f, 0.8f, 0.0f), forward, obstacleDistance) ||
...
bool TrafficSystem::PlaceOnGround(TrafficCar& car, PhysicsWorld& city, bool immediate) {
    ...
    if (!city.RaycastStatic(centerOrigin, glm::vec3(0.0f, -1.0f, 0.0f), centerDistance) || centerDistance > 12.0f ||
        !city.RaycastStatic(frontOrigin, glm::vec3(0.0f, -1.0f, 0.0f), frontDistance) || frontDistance > 12.0f) {
        return false;
    }
...
void TrafficSystem::Update(float deltaTime, PhysicsWorld& city, const glm::vec3& userPosition, const glm::vec3& userForward) {
    ...
    for (TrafficCar& car : cars) {
        ...
        if (!recycle && car.obstacleCheckTimer <= 0.0f) {
            ...
            recycle = city.RaycastStatic(car.position + glm::vec3(0.0f, 0.8f, 0.0f) + forward * 1.4f, forward, obstacleDistance) &&
    ...
    UpdateCollisionBoxes(city);
}

void TrafficSystem::UpdateCollisionBoxes(PhysicsWorld& city) const {
    std::vector<DynamicCollisionBox> boxes;
    boxes.reserve(cars.size());
    ...
    for (const TrafficCar& car : cars) {
        ...
        DynamicCollisionBox box;
        box.center = car.position + glm::vec3(0.0f, kTrafficCollisionSize.y * 0.5f, 0.0f);
        box.size = kTrafficCollisionSize;
        box.yaw = car.yaw;
        boxes.push_back(box);
    }

    city.SetDynamicCollisionBoxes(boxes);
}
